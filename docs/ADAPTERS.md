...
**12. A seam and its only implementation, never joined — found by audit, not by a drill.** Unlike the eleven above, this one no drill could catch, because the code involved never ran outside its own unit tests. `ReversalEngine` had accepted a `command_classifier` since the seam was designed, and `revoco.reversal.shell` had implemented one — 27KB of reach analysis, 22 escape rules, ~70 read-only commands. Nothing in `src/` or `scripts/` ever passed the second to the first. Both halves had tests, both passed, and the product could not reach either. The workstation validator hand-wires `WORKSPACE_SPEC` to a fixed tool name, which drills the *snapshot* but never the *classifier* that decides which commands deserve one. **Test coverage measures whether code works, not whether anything calls it**; an import-reachability pass over `src/` and `scripts/` takes seconds and answers a question the suite structurally cannot. This is the fifth instance of built-but-unreachable in this codebase, and the first four were all found by accident.

**13. The same audit, one module further: a component with no way to start it.** `revoco.mcp.proxy` is a working stdio MCP proxy — 291 lines, 13 tests — and a stdio proxy's *only* mode of use is being launched as a subprocess named in a client's config. It had no `[project.scripts]` entry, no `__main__`, and no mention in any document, so there was no command anyone could put in that config. Worse, the README stated the opposite as a live claim: that `mcp-gate` still ships a proxy "where revoco has no MCP integration at all". The catalogue omission in finding 12's neighbourhood under-reported a mechanism; this one **asserted the absence of a thing that was present**. A reachability audit finds the first; only reading the prose against the tree finds the second.

**14. The report that says a control got worse could not say it about this one.** `compare()` checked whether the *current* outcome was `NOT_DRILLABLE` before it consulted the previous one, so a control that passed last run and has nothing to prove this run landed in the benign bucket. Deleting the classifier from the validator to check finding 12's fix produced exactly that: the suite printed `10/10 drills passed`, `validation-report` said **"no control got worse"**, and the exit code was `0` — while a drill had stopped testing anything. `DISAPPEARED` already existed for a control that leaves the suite, and its docstring states the principle — *"untested is the state this whole package exists to stop being mistaken for safe"* — but a control that stays listed and stops proving is the **quieter** version of the same harm, and it was the one case the alarm could not reach. It is now a `REGRESSED`; a control that was never drillable stays benign, so `fs.read_file` does not cry wolf.
