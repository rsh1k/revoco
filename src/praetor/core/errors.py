...
for a control plane specifically: a caller wrapping ``praetor`` needs one
``except PraetorError`` to be sure it cannot leak an unhandled exception into a
fail-open path.
"""
...
class PraetorError(Exception):
    """Base class for every error raised by praetor."""
...
class PolicyError(PraetorError):
    """A policy document is invalid and cannot be loaded."""


# ---- authority errors -----------------------------------------------------
class AuthorityError(PraetorError):
...
# ---- ledger errors --------------------------------------------------------
class TamperDetected(PraetorError):
    """The hash chain does not validate: history has been altered."""


# ---- reversal errors ------------------------------------------------------
class ReversalError(PraetorError):
